...
def fct_calcul_pressure_exerted_by_an_inters_control_stage(val_intersection,val_inters_stage,val_network,di_id_phase_val_qweight,\
val_round_prec):
...
val_param_comparaison,val_di_id_phase_val_qweight,val_round_precision,val_init_small_val):
	...
	val_network=val_netw,di_id_phase_val_qweight=val_di_id_phase_val_qweight,val_round_prec=val_round_precision)
	
	fct_select_inters_control_stage_exerting_higher_pressure(\
	va_intersection=val_intersect,va_network=val_netw,va_di_id_phase_val_qweight=val_di_id_phase_val_qweight,\
...
v_t_end_current_network_control_matrix,v_t_unit,v_dt,v_init_small_val=-10**10):
	...
	t_end_current_icm_rc=round(t_start_red_clear+li_param_control[1]-v_t_unit,v_t_round_prec)
	...
	t_end_current_icm=round(t_start+durat_ctrl-v_t_unit,v_t_round_prec)
	...
	val_t_end_control= t_end_current_icm_rc,val_duration_control=li_param_control[1],\
	...
	val_type_control=Cl_Control_Actuate.TYPE_CONTROL[13],\
	val_type_control_related_to_t_revision=Cl_Control_Actuate.TYPE_CONTROL_T_REVISION_CATEGORY["without_sensor_requirement_for_t_update"],\
	val_t_end_control= t_end_current_icm,val_duration_control=durat_ctrl,\
	val_estim_turn_ratios_with_current_ctrl=v_network.get_di_intersections()[v_intersection.get_id_node()].get_estimated_turn_ratios(),\
...
#v_li_param_control=[actuation durationn of a stage, duration of red clearance, 
#parameter comparison exerted pressure betwenne current stage and new selected stage]
#,val_dic_id_phase_val_qweight=dict, key=id phase, value= q value

def admissible_intersection_control_object_next_period_mp_pract(t_actuel,t_start_sim,v_sim_dur,v_inters,v_netwk,v_li_param_control,v_round_precis,\
v_t_round_precis,v_t_end_current_netwk_control_matrix,v_ti_unit,v_marge_dt,val_dic_id_phase_val_qweight,v_initial_small_val=-10**10):
	...
	if t_actuel==t_start_sim:	
		...
		va_di_id_phase_val_qweight=val_dic_id_phase_val_qweight,\
		...
		if fct_defining_whether_new_stage_employed(val_intersect=v_inters,val_netw=v_netwk, val_exerted_pres_new_selected_stage=li_select_stage_info[2],\
		val_param_comparaison=v_li_param_control[2],\
		val_di_id_phase_val_qweight=val_dic_id_phase_val_qweight,\
		val_round_precision=v_round_precis,val_init_small_val=v_initial_small_val)==1:
			...
			v_dt=v_marge_dt,\
			v_t_unit=v_ti_unit,\
			v_init_small_val=v_initial_small_val)
