...
#di_id_phase_val_qweight=dict, key=phase id, value=the Q(l,m) for the weighted ques
def fct_calcul_pressure_exerted_by_an_inters_control_stage(val_intersection,val_inters_stage,val_network,val_round_prec,di_id_phase_val_qweight):
	
	pres_exerted=0
	


	#print("di_id_phase_val_qweight in algo",di_id_phase_val_qweight)
	...
	for i in val_inters_stage:
		...
		if val_network.get_di_all_links()[i[1]].get_type_network_link() != Cl_Network_Link.TYPE_NETWORK_LINK["exit"]:
			...
			for j in val_network.get_di_entry_internal_links()[i[1]].get_set_veh_queue().get_di_obj_veh_queue_at_link():
				...
				get_estimated_turn_ratios()==1:
					...
					#the product of the turn ratio x queue length (gamma(m,p) x q(m,p) x Q(m,p)^2)
		...
		weight_queue=(\
		(le*di_id_phase_val_qweight[i[0],i[1]]**2-som)*val_network.get_di_entry_internal_links()[i[0]].get_set_veh_queue().get_di_obj_veh_queue_at_link()[i[0],i[1]].\
		get_sat_flow_queue())
...
def fct_select_inters_control_stage_exerting_higher_pressure(va_intersection,va_network,va_round_prec,va_init_small_val,va_di_id_phase_val_qweight):
...
def admissible_intersection_control_object_next_period_mp(v_intersection,v_network,li_param_control,v_round_prec,v_t_round_prec,\
...
###########################computations for the red clearance ######################################################		
	...
	t_upd_ctrl=fct_calcul_t_update_ctrl(va_t_end_last_control_of_seq_ctrls=t_end_current_icm_rc, va_dt=v_dt,va_t_round_precision=v_t_round_prec,\
	va_t_unit=v_t_unit)
############################################################################################################ 
	...
	val_type_control=Cl_Control_Actuate.TYPE_CONTROL[3],\
	...
	val_id_actuated_stage=mp_inters_control_stage[1],\
	val_cycle_duration_associated_with_control=duration_cycle,val_t_update_ctrl=t_upd_ctrl)
	...
	val_estim_turn_ratios_with_current_ctrl=v_network.get_di_intersections()[v_intersection.get_id_node()].get_estimated_turn_ratios(),\
	val_cycle_duration_associated_with_control=duration_cycle,\
