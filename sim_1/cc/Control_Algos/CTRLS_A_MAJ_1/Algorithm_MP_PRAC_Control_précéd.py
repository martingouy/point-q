...
def fct_calcul_pressure_exerted_by_an_inters_control_stage(val_intersection,val_inters_stage,val_network,val_round_prec):
	...
	for i in val_inters_stage:
		...
		if val_intersection.get_id_node()==37593:
			print("len:",le,"som",som,"weight que",weight_queue,"pres_exerted",pres_exerted)
			print()
...
def fct_calcul_weight_phase(val_id_phase,val_network):
	...
	#weight  phase =sum sur actuated (l,m) [q(l,m) - sum sur p (r(m,p) x gamma(m,p))]
	
	print("calcul weight phase: ", val_id_phase)
	#queue length q(l,m)
	...
	"weigh phase: ",(le-som)*val_network.get_di_entry_internal_links()[val_id_phase[0]].get_set_veh_queue().get_di_obj_veh_queue_at_link()[val_id_phase[0],val_id_phase[1]].get_sat_flow_queue() )
	return \
	(le-som)*val_network.get_di_entry_internal_links()[val_id_phase[0]].get_set_veh_queue().get_di_obj_veh_queue_at_link()[val_id_phase[0],val_id_phase[1]].get_sat_flow_queue()
...
def fct_def_whether_MP_stage_update_1(val_netw,val_id_inters):
...
def fct_def_whether_MP_stage_update(val_netw,val_id_inters):
	...
	if val_netw.get_di_intersections()[val_id_inters].get_intersection_control_obj().get_id_actuated_stage() !=-1:
		...
		#for chaque phase actuated by the current ctrl, (l,m), we calculate the [ sat.flow (l,m) -q(l,m)) ] x weight phase
		#PEUT ETRE PRENDRE EN COMPTE LES VEH QUI SONT EN TRAIN DE PARCOURIR L'ARC VERS UNE QUE
		for j in val_netw.get_di_intersections()[val_id_inters].get_di_stages_sign_intersection()[\
		val_netw.get_di_intersections()[val_id_inters].get_intersection_control_obj().get_id_actuated_stage()]:
			...
			som+=(val_netw.get_di_entry_internal_links()[j[0]].get_set_veh_queue().get_di_obj_veh_queue_at_link()[j[0],j[1]].get_sat_flow_queue() -\
			len(val_netw.get_di_entry_internal_links()[j[0]].get_set_veh_queue().get_di_obj_veh_queue_at_link()[j[0],j[1]].get_queue_veh()))*\
			fct_calcul_weight_phase(val_id_phase=[j[0],j[1]],val_network=val_netw)
			
		if som>0:
			print("IN MP P, NO UPDATE som in MP P",som,"id stage cour",val_netw.get_di_intersections()[val_id_inters].get_intersection_control_obj().get_id_actuated_stage())
			print()
			return 0
	
		else:
			print("IN MP P, UPDATE som in MP P",som,"id stage cour",val_netw.get_di_intersections()[val_id_inters].get_intersection_control_obj().get_id_actuated_stage())
			print()
			return 1
	#if the current stage is RC
	else:
...
def admissible_intersection_control_object_next_period_mp_sans_rc(v_intersection,v_network,di_param_control,v_round_prec,v_t_round_prec,\
v_t_end_current_network_control_matrix,v_t_unit,v_dt,v_init_small_val=-10**10):
	...
	#indice choix list stage actuat durat and red cleat
	...
	#print("indice_choix_duree",indice_choix_duree,di_param_control[v_intersection.get_id_node()][0][indice_choix_duree])
	
	li_stage_actuation_duration_and_idle_time=di_param_control[v_intersection.get_id_node()][0][indice_choix_duree]
	
	
	#si le control actuel est different du nouveau on cree une matrice RC
	if mp_inters_control_stage_and_id_stage[1]!=v_intersection.get_intersection_control_obj().get_id_actuated_stage():
	
		
		#calcul duree red clear
		
		#si on commence une nouvelle serie, red clear est la red clear de la derniere icm de la serie
		if indice_choix_duree==1:
			
			indice_choix_duree_red_clearance=di_param_control[v_intersection.get_id_node()][2]-1
		
			li_stage_actuation_duration_and_idle_time_red_clear=di_param_control[v_intersection.get_id_node()][0][indice_choix_duree_red_clearance]
		
			duration_idle_time=li_stage_actuation_duration_and_idle_time_red_clear[1]
			
		#si on continue une serie, red clear est le (nb ctrl-1) (on prend le duree de rc de la matriceprec)
		else:
			indice_choix_duree_red_clearance=di_param_control[v_intersection.get_id_node()][3]+1
		
			duration_idle_time=li_stage_actuation_duration_and_idle_time[1]
		#if v_intersection.get_id_node()==37593:
			#print("duration_idle_time",duration_idle_time)
	
		################## computation times red clear ##############################
	
		t_start_red_clear=round(v_t_end_current_network_control_matrix+v_t_unit,v_t_round_prec) 
		...
		li_ico=[inters_control_obj_rc,inters_control_obj_mp]
		
		
	#si le control actuel est le même
	else:
...
v_t_end_current_netwk_control_matrix,v_ti_unit,v_marge_dt,v_initial_small_val=-10**10):
	...
	if t_actuel==t_start_sim:	
		...
		#on increment de un l'indice de choix des durees	
		#di_param_control=dict, key = id node , value=list[list stage actuat dura and red clear dura, cycle duration, len(list stage actuat dura and red clear dura),\
		#indice choix list stage actuat dura and red cleat
		...
		if fct_def_whether_MP_stage_update(val_netw=v_netwk,val_id_inters=v_inters.get_id_node())==1:
			...
			return admissible_intersection_control_object_next_period_mp_sans_rc(\
