...
def fct_define_ctrl_amongst_all_stages(t_act,v_inters,v_netw,v_li_param_ctrl,v_ti_unit,v_dt,v_round_precis,v_t_round_prec,v_initial_small_val):
	...
	if v_inters.get_intersection_control_obj()!=None:
		...
		if rep[1]!=v_inters.get_intersection_control_obj().get_id_actuated_stage():
			...
			t_end_rc=round(t_start_rc+v_li_param_ctrl[1]-v_t_unit,v_t_round_prec)
			
			t_start=round(t_end_rc+v_t_unit,v_t_round_prec)
			
			t_end=fct_calcul_t_update_ctrl_fa_mg(va_t_start_ctrl=t_start, va_max_dur=v_li_param_ctrl[2],va_t_unit=v_t_unit,va_t_round_precision=v_t_round_prec)
