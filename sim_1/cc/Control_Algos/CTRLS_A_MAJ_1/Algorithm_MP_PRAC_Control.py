...
def fct_defining_whether_new_stage_employed(val_netw, val_exerted_pres_new_selected_stage,\
val_param_comparaison,va_round_prec,val_init_small_val):

	#calcul of the exerted pression by the currently employed stage
	pres_cur_stage=fct_select_inters_control_stage_exerting_higher_pressure(\
	va_intersection=val_intersect,va_network=val_netw,va_round_prec=val_round_prec,va_init_small_val=val_init_small_val)
	
	if pres_cur_stage - val_exerted_pres_new_selected_stage > val_param_comparaison:
		return 1
	else:
		return 0
...
di_param_control,v_round_prec,v_t_round_prec,\
...
#on retourne une liste d' inters control objet and le type du prochain controle a appliquer par le (prochain) even en_decision_icm
#duration_icm= le temps d'actualisation de l'icm
#li_param_MP_control=[...,[intersection stage actuat duration ith period, durat red,clear, val_paramet_select_new_stage],..., cycle duration]
...
def admissible_intersection_control_object_next_period_mp_pract(t_actuel,t_start_sim,v_sim_dur,v_inters,v_netwk,v_di_param_control,v_round_precis,\
v_t_round_precis,v_t_end_current_netwk_control_matrix,v_ti_unit,v_marge_dt,v_initial_small_val=-10**10):
	...
	if t_actuel==t_start_sim:	
		...
		#print("di_param_control[v_intersection.get_id_node()]",di_param_control)
		indice_choix_duree=v_di_param_control[v_inters.get_id_node()][4]+1	
		...
		if indice_choix_duree==v_di_param_control[v_inters.get_id_node()][3]-1:
			indice_choix_duree=-1
			
		#on maj l'indice  pour le prochain choix de durees dans control actuate objet
		#v_network.get_control_actuate_obj().get_di_param_mp_ctrl()[v_intersection.get_id_node()][3]=indice_choix_duree
		v_di_param_control[v_inters.get_id_node()][4]=indice_choix_duree
		...
		if fct_defining_whether_new_stage_employed(val_netw=v_netwk, val_exerted_pres_new_selected_stage=li_select_stage_info[2],\
		val_param_comparaison=v_di_param_control[v_inters.get_id_node()][4],va_round_prec=v_round_precis,val_init_small_val=v_initial_small_val)==1:
			...
			di_param_control=v_di_param_control,v_round_prec=v_round_precis,\
