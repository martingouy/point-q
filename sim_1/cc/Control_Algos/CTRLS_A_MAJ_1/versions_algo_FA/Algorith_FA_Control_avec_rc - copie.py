...
#REMARK : THIS ALGO FUNCTIONS WHEN THERE EXIST ONLY TWO CANDIDATE STAGES 
#EACH STAGE ACTUAT INE PHASE PER INTERSECTION
#EACH STAGE ACTUATING ONE PHASE (l,m)
...
def fct_select_product_que_and_sat_flow_single_stage(va_intersection,va_stage,va_network,va_round_precis):
	...
	for i in va_stage:
		...
		for k in va_network.get_di_entry_internal_links()[i[0]].get_set_veh_queue().get_di_obj_veh_queue_at_link():
			...
			[k[0],k[1]].get_queue_veh())*va_network.get_di_entry_internal_links()[i[0]].get_set_veh_queue().get_di_obj_veh_queue_at_link()\
...
def fct_select_intersection_stage_with_max_prod(va_inters,va_netw,va_round_precision,va_init_small_val):
	...
	for i in li_id_stages_inersection:
		...
		current_exerted_pression=fct_select_product_que_and_sat_flow_single_stage(va_intersection=va_inters,\
...
def admissible_intersection_control_object_fa(t_actuel,v_intersection,v_network,li_param_control,v_sim_dur,v_t_unit,v_t_round_prec,\
	...
	#si on ne commence pas la sim, li_param_control=[v_admissible_limit_flow, idle time, t_prem_ctrl]
		...
		if v_intersection.get_intersection_control_obj().get_type_control()!=Cl_Control_Actuate.TYPE_CONTROL[0]:
			...
			if fa_stage[1]!=v_intersection.get_intersection_control_obj().get_id_actuated_stage():
				...
				return[li_ico, List_Explicit_Values.initialisation_value_to_ten,List_Explicit_Values.initialisation_value_to_zero]
		...
		#si on est en red clear
			...
			t_start=round(v_intersection.get_intersection_control_obj().get_t_end_control()+v_t_unit,v_t_round_prec)
			t_end=round(t_start+v_sim_dur-v_t_unit,v_t_round_prec)
