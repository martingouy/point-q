...
#method returning the  sum of product que length x sat flow of all the  queues actuated by the stage
def fct_select_product_que_and_sat_flow_single_stage(va_intersection,va_stage,va_network,va_round_precis):
	...
	if va_intersection.get_id_node()==37593:
		print("va_stage",va_stage)
		
	#par example un inters stage [[7,8],[9,10]] actuat simult phases [7,8] et [9,10]
	for i in va_stage:
		if va_intersection.get_id_node()==37593:
			print("que stage=",i)
			
		#pour chaque que activee par le  stage, on calcule le produit  queue length x sat flow of the queue
		...
		som+=prod
		print("que length",len(va_network.get_di_entry_internal_links()[i[0]].get_set_veh_queue().get_di_obj_veh_queue_at_link()\
		[i[0],i[1]].get_queue_veh()),"sat flow",va_network.get_di_entry_internal_links()[i[0]].get_set_veh_queue().get_di_obj_veh_queue_at_link()\
		[i[0],i[1]].get_sat_flow_queue(),"som",som)
	print()
...
#li_param_control=[v_admissible_limit_flow,  t_prem_ctrl_start_sim]
...
def admissible_intersection_control_object_fa_no_rc(t_actuel,v_intersection,v_network,li_param_control,v_sim_dur,v_t_unit,v_t_round_prec,\
v_init_small_val=-10**10):
	...
	if t_actuel==li_param_control[1]:
		...
		#creation d'un objet intersection control, pour la RC, de duree li_param_control[1]
		...
		val_type_control=Cl_Control_Actuate.TYPE_CONTROL[10],\
		val_id_actuated_stage=id_stage)
		
		return[[inters_control_obj], List_Explicit_Values.initialisation_value_to_ten,List_Explicit_Values.initialisation_value_to_zero]
	
	#si on ne commence pas la sim, li_param_control=[v_admissible_limit_flow,  t_prem_ctrl]
	elif t_actuel>li_param_control[1]:
		...
		if v_intersection.get_id_node()==37593:
			print("id selected stage:",fa_stage[1], "id current stage",v_intersection.get_intersection_control_obj().get_id_actuated_stage())
			print()
		...
		if fa_stage[1]!=v_intersection.get_intersection_control_obj().get_id_actuated_stage():
			...
			return[[inters_control_obj_fa], List_Explicit_Values.initialisation_value_to_ten,List_Explicit_Values.initialisation_value_to_zero]
		...
		print("PROBLEM DS ALGORITHM_FA_CONTROL, t_act, t_first_ctrl", t_actuel,li_param_control[1])
		import sys
		sys.exit()
