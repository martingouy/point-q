...
#DYNAMICALLY MEASURED ROUT PROB
#*****************************************************************************************************************************************************************************************
...
def fct_calcul_pressure_exerted_by_an_inters_control_stage(val_intersection,val_inters_stage,val_network,val_round_prec):
	...
	#on va mesurer les prob de routage de chaque queue de chaque output arc de l'intersection
	di_key_id_que_value_rout_prob={}
	#pour chaque output arc
	for  m in val_intersection.get_li_id_output_network_links_from_inters_node():
	
		if  val_network.get_di_all_links()[m].get_type_network_link() != Cl_Network_Link.TYPE_NETWORK_LINK["exit"]:
			som_ar=0
		
			#pour chaque queue de l'output arc, dict, key=[id input, output lk], value=queue object
			for n in val_network.get_di_entry_internal_links()[m].get_set_veh_queue().get_di_obj_veh_queue_at_link():
				som_ar+=val_network.get_di_entry_internal_links()[m].\
				get_set_veh_queue().get_di_obj_veh_queue_at_link()[n].get_nb_veh_arrived_current_period()
			
			for p in val_network.get_di_entry_internal_links()[m].get_set_veh_queue().get_di_obj_veh_queue_at_link():
			
				if  som_ar>0:
					di_key_id_que_value_rout_prob[p[0],p[1]]=round(val_network.get_di_entry_internal_links()[m].\
					get_set_veh_queue().get_di_obj_veh_queue_at_link()[p].get_nb_veh_arrived_current_period()/som_ar,val_round_prec)
				else:	
					di_key_id_que_value_rout_prob[p[0],p[1]]=val_network.get_di_intersections()\
					[val_network.get_di_entry_internal_links()[m].get_id_head_intersection_node()].\
					get_di_key_id_phase_value_li_rout_prob()[p[0],p[1]][0] 
			
				if val_intersection.get_id_node()==5:
					print("AVANT",p,val_network.get_di_entry_internal_links()[m].\
					get_set_veh_queue().get_di_obj_veh_queue_at_link()[p].get_nb_veh_arrived_current_period(),\
					di_key_id_que_value_rout_prob[p[0],p[1]])
	...
	for i in val_inters_stage:
		...
		if val_network.get_di_all_links()[i[1]].get_type_network_link() != Cl_Network_Link.TYPE_NETWORK_LINK["exit"]:
			...
			for j in val_network.get_di_entry_internal_links()[i[1]].get_set_veh_queue().get_di_obj_veh_queue_at_link():
				...
				a=di_key_id_que_value_rout_prob[j[0],j[1]] *\
...
def admissible_intersection_control_object_next_period_mp_mod(v_intersection,v_network,di_param_control,v_round_prec,v_t_round_prec,\
v_t_end_current_network_control_matrix,v_t_unit,v_init_small_val=-10**10):
	
	print("here algo avant")
	...
	#we update the values of vehicle arrival for every  queue of the output lins of the intersection
	for  m in v_intersection.get_li_id_output_network_links_from_inters_node():
		if  v_network.get_di_all_links()[m].get_type_network_link() != Cl_Network_Link.TYPE_NETWORK_LINK["exit"]:
		
			#pour chaque queue de l'output arc, dict, key=[id input, output lk], value=queue object
			for n in v_network.get_di_entry_internal_links()[m].get_set_veh_queue().get_di_obj_veh_queue_at_link():
				v_network.get_di_entry_internal_links()[m].\
				get_set_veh_queue().get_di_obj_veh_queue_at_link()[n].\
				set_nb_veh_arrived_current_period(List_Explicit_Values.initialisation_value_to_zero)
				if v_intersection.get_id_node()==5:
					print("APRES",n,v_network.get_di_entry_internal_links()[m].\
					get_set_veh_queue().get_di_obj_veh_queue_at_link()[n].get_nb_veh_arrived_current_period())
	...
	return[li_ico, List_Explicit_Values.initialisation_value_to_twelve,List_Explicit_Values.initialisation_value_to_one]
