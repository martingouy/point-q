...
#*****************************************************************************************************************************************************************************************
	...
	def fct_insertion_even_in_event_list1(self,ev,event_list,message):
		...
		if len(event_list) > List_Explicit_Values.initialisation_value_to_zero:
			...
			if ev.get_event_time() < event_list[List_Explicit_Values.val_first_element_of_list].get_event_time():
				print("IN FUNCTION INSERTION EVENT, TIME NEW EVENT",ev.get_event_time(),"TIME FIRST EVENT IN THE LIST: ",event_list[List_Explicit_Values.val_first_element_of_list].get_event_time())
				print (message)
				import sys
				sys.exit()
...
#*****************************************************************************************************************************************************************************************
	...
	def fct_calcul_nb_veh_they_can_leave_at_t_current_and_assoc_end_dep_time_mi_without_permis_mv1_1(self,v_t_cur,v_t_unit,v_queue,\
	v_min_veh_hold_time_in_que,v_t_end_current_control,v_netw,val_vect_nb_veh_to_exam,val_round_prec):
		...
		if v_netw.get_di_all_links()[v_queue.get_id_associated_output_link()].get_type_network_link()!=Cl_Network_Link.TYPE_NETWORK_LINK["exit"]:
			...
			#if we consider infinity link capacity
			if v_netw.get_di_entry_internal_links()[v_queue.get_id_associated_output_link()].get_capacity_link() ==-1:
				
				#the potential number of vehicles that can leave que
				total_potential_nb_dep_veh= v_queue.get_current_queue_service_rate() - v_queue.get_current_reached_service_rate()
				
			
				total_potential_nb_dep_veh_1=min(total_potential_nb_dep_veh,val_vect_nb_veh_to_exam[0])
				...
				nb_veh_have_rem_que_hold_time_without_plan_dep=\
				v_queue.fct_calcul_nb_of_n_veh_remained_in_que_for_min_t_hold_without_planned_dep_for_mi(t_current=v_t_cur,\
				min_hold_time=v_min_veh_hold_time_in_que,li_vehicles=v_queue.get_queue_veh()[:total_potential_nb_dep_veh_1],\
				nb_veh_to_exam=val_vect_nb_veh_to_exam[0],val_precision=val_round_prec)
				...
				t_left=round(v_t_end_current_control - v_t_cur,val_round_prec) 
		
				#the number of vehicles that can leave within t_left seconds
				#nb_veh_can_leave_during_t_left=math.ceil(queue.get_sat_flow_queue() * t_left/t_unit )
				nb_veh_can_leave_during_t_left=round(v_queue.get_sat_flow_queue() * t_left/v_t_unit )
				...
				t_req=round(nb_veh_leave*v_t_unit/v_queue.get_sat_flow_queue(),val_round_prec)
				if t_req>t_left:
					t_req=t_left
		
				#if t_req>t_left:
					#print("PROBLEM IN CL_GLOBAL_FUNCTIONS, IN fct_calcul_nb_veh_they_can_leave_at_t_current_and_assoc_end_dep_time_mi,t_req:", t_req,"t_left",t_left,"que:",queue.get_associated_phase_to_queue(),"nb veh leave:",nb_veh_leave,\
					#"t_unit:",t_unit,"s:",queue.get_sat_flow_queue(),nb_veh_leave*t_unit/queue.get_sat_flow_queue())
					#import sys
					#sys.exit()
				t_ev_end_veh_dep=round(v_t_cur+t_req,val_round_prec)
				
				#we indicate that the dest lk is not saturated
				state_destin_link=List_Explicit_Values.initialisation_value_to_zero
			
			#if we consider limited link capacity
				...
				#if val_netw.get_di_entry_internal_links()[queue.get_id_associated_output_link()].\
				#get_nb_avail_link_places()>0:
				nb_avail_places_dest_lk=\
				v_netw.get_di_entry_internal_links()[v_queue.get_id_associated_output_link()].get_capacity_link() - \
				v_netw.get_di_entry_internal_links()[v_queue.get_id_associated_output_link()].get_current_nb_veh_link()
				...
				if nb_avail_places_dest_lk>0 :
					...
					total_potential_nb_dep_veh_1=min(total_potential_nb_dep_veh,val_vect_nb_veh_to_exam[0],nb_avail_places_dest_lk)
			...
			#"total_potential_nb_dep_veh_1=",total_potential_nb_dep_veh_1,\
			#"nb_veh_have_rem_que_hold_time_without_plan_dep=",nb_veh_have_rem_que_hold_time_without_plan_dep,"t_left=",t_left,\
			#"nb_veh_can_leave_during_t_left=",nb_veh_can_leave_during_t_left,"nb_veh_leave=",nb_veh_leave)
			#import sys
			#sys.exit()
		...
		#if v_t_cur==449.1 and  v_queue.get_id_associated_link()==6 and v_queue.get_id_associated_output_link()==7:
			#print(nb_veh_leave,t_ev_end_veh_dep,total_potential_nb_dep_veh,state_destin_link)
			#print(nb_veh_have_rem_que_hold_time_without_plan_dep)
			#import sys
			#sys.exit()
...
#*****************************************************************************************************************************************************************************************
	...
	def fct_calcul_nb_veh_they_can_leave_at_t_current_and_assoc_end_dep_time_mi_without_permis_prior_mv_infinite_lk_cap_1(self,\
	 ...
	 va_queue,va_vect_nb_veh_to_exam,va_min_veh_hold_time_in_que,va_t_round_prec):
	 
		#the potential number of vehicles that can leave que from the sat flow
		...
		#nb_veh_to_exam=va_vect_nb_veh_to_exam[0],val_precision=va_round_prec)
		nb_veh_have_rem_que_hold_time_without_plan_dep=\
		va_queue.fct_calcul_nb_of_n_veh_remained_in_que_for_min_t_hold_without_planned_dep_for_mi(t_current=va_t_cur,\
		min_hold_time=va_min_veh_hold_time_in_que,li_vehicles=va_queue.get_queue_veh()[:total_potential_nb_dep_veh_1],\
		...
		#the number of vehicles that can leave within t_left seconds
		#nb_veh_can_leave_during_t_left=math.ceil(queue.get_sat_flow_queue() * t_left/t_unit )
		nb_veh_can_leave_during_t_left=round(va_queue.get_sat_flow_queue() * t_left/va_t_unit )
		
		
		if nb_veh_can_leave_during_t_left==List_Explicit_Values.initialisation_value_to_zero:
		
			#the required time for nb_veh_have_rem_que_hold_time_without_plan_dep to leave
			t_req=round((va_t_unit *nb_veh_have_rem_que_hold_time_without_plan_dep)/va_queue.get_sat_flow_queue(),va_t_round_prec)
			
			veh_will_leave=self._decision_obj.\
			fct_dec_whether_veh_depart_by_end_ctrl_act(val_t_remain=t_left,val_t_required=t_req,a=0,b=1,val_t_round_prec=2)
			...
			#the required time for nb_veh_leave to leave
			t_req=round(nb_veh_leave*va_t_unit/va_queue.get_sat_flow_queue(),va_t_round_prec)
			
		if t_req>t_left:
			t_req=t_left
		
		t_ev_end_veh_dep=round(va_t_cur+t_req,va_t_round_prec)
...
#*****************************************************************************************************************************************************************************************
	...
	def fct_calcul_nb_veh_they_can_leave_at_t_current_and_assoc_end_dep_time_mi_without_permis_prior_mv_infinite_lk_cap(self,\
	 ...
	 va_queue,va_min_veh_hold_time_in_que,va_t_round_prec):
		...
		print("existance veh in queue avec hold time ok",nb_veh_have_rem_que_hold_time_without_plan_dep>List_Explicit_Values.initialisation_value_to_zero)	
		...
		if nb_veh_have_rem_que_hold_time_without_plan_dep>List_Explicit_Values.initialisation_value_to_zero:
			...
			#If the required time > t_left
			if t_left< va_queue.get_required_depart_time_que():
				print("t_left",t_left,"T NECES",va_queue.get_required_depart_time_que())
	 ...
	 va_queue,va_vect_nb_veh_to_exam,va_min_veh_hold_time_in_que,va_t_round_prec):
		...
		if nb_avail_places_dest_lk>0 :
			...
			if nb_veh_have_rem_que_hold_time_without_plan_dep>List_Explicit_Values.initialisation_value_to_zero:
				...
				#the number of vehicles  which can leave within t_left seconds
				#nb_veh_can_leave_during_t_left=math.ceil(queue.get_sat_flow_queue() * t_left/t_unit )
				nb_veh_can_leave_during_t_left=round(va_queue.get_sat_flow_queue() * t_left/va_t_unit )
				
				
				#if  vehicles cannot leave within the t left
				if nb_veh_can_leave_during_t_left==List_Explicit_Values.initialisation_value_to_zero:
					
					
					#the required time for nb_veh_have_rem_que_hold_time_without_plan_dep to leave
					#t_req=round((va_t_unit *nb_veh_have_rem_que_hold_time_without_plan_dep)/va_queue.get_sat_flow_queue(),va_t_round_prec)
					duree_init=va_t_unit *nb_veh_have_rem_que_hold_time_without_plan_dep/va_queue.get_sat_flow_queue()
					duree_init_multipliee=duree_init*100
					...
					if partie_decimale==0:
						...
						t_req=(((va_t_unit *nb_veh_have_rem_que_hold_time_without_plan_dep)/va_queue.get_sat_flow_queue())*100//1)/10
				...
				#if vehicles can leave with the t left
				else:
			...
			#if  no vehicle has remained the min time in the queue (or if que is empty)
			else:
				nb_veh_leave=List_Explicit_Values.initialisation_value_to_zero
...
#*****************************************************************************************************************************************************************************************
	#method eturning the nb of vehicles that can leave from a queue in case where prior-minor phases are not considered,
	#in case of finite capacity of the destination link
	def  fct_calcul_nb_veh_they_can_leave_at_t_current_and_assoc_end_dep_time_mi_without_permis_prior_mv_finite_lk_cap_2(self,\
	 va_t_end_current_control,va_t_cur,va_t_unit,va_netw,\
	 va_queue,va_min_veh_hold_time_in_que,va_t_round_prec):
...
#method eturning the nb of vehicles that can leave from a queue in case where prior-minor phases are not considered,
	#in case of finite capacity of the destination link
	def  fct_calcul_nb_veh_they_can_leave_at_t_current_and_assoc_end_dep_time_mi_without_permis_prior_mv_finite_lk_cap_3(self,\
	 va_t_end_current_control,va_t_cur,va_t_unit,va_netw,\
	 va_queue,va_min_veh_hold_time_in_que,va_t_round_prec):
...
#*****************************************************************************************************************************************************************************************
	...
	#method returning the number of vehicles that can depart from a permissive (minor) que, the associated dep time,total_potential_nb_dep_veh
	#val_vect_nb_veh_to_exam=[nb veh permitted to go ny the sat flow, t_req]
	def fct_calcul_nb_veh_they_can_leave_at_t_current_and_assoc_end_dep_time_mi_from_permis_mv(self,\
	va_t_current,va_t_unit, va_t_end_current_control,va_min_veh_hold_time_in_que, \
	va_id_input_lk_minor_phase,va_id_output_lk_minor_phase,va_id_input_lk_prior_phase,va_id_output_lk_prior_phase,\
	va_network,va_vect_nb_veh_to_exam,va_t_round_prec):
	
		#the required time for a single vehicle of the minor phase to cross the inters (= to the required cross time for the number of vehicles
		#which can cross the  intersection defined by the sat. flow of the phase)
		#t_req_single_veh_cross_inters_minor_phase=round(v_t_unit/val_network.get_di_entry_internal_links()[\
		#val_id_input_lk_minor_phase].get_set_veh_queue().get_di_obj_veh_queue_at_link()[val_id_input_lk_minor_phase,\
		#val_id_output_lk_minor_phase].get_sat_flow_queue(),val_round_prec)
		
		#the required time for a single vehicle of the prior phase to cross the inters (= to the required cross time for the number of vehicles
		#which can cross the  intersection defined by the sat. flow of the phase)
		#t_req_single_veh_cross_inters_prior_phase=round(v_t_unit/val_network.get_di_entry_internal_links()[\
		#val_id_input_lk_prior_phase].get_set_veh_queue().get_di_obj_veh_queue_at_link()[val_id_input_lk_prior_phase,\
		#val_id_output_lk_prior_phase].get_sat_flow_queue(),val_round_prec)
		
		
		#we examine if a (single) veh can leave from the permis movn supposing thetw ctrl authorises it
		veh_can_leave_permis_mv=self.fct_exam_whether_vehs_minor_mov_can_leave(\
		t_current=va_t_current,\
		val_inters=va_network.get_di_intersections()[va_network.get_di_entry_internal_links()[va_id_input_lk_minor_phase].get_id_head_intersection_node()],\
		val_minor_que=va_network.get_di_entry_internal_links()[\
		va_id_input_lk_minor_phase].get_set_veh_queue().get_di_obj_veh_queue_at_link()[va_id_input_lk_minor_phase,\
		va_id_output_lk_minor_phase],\
		val_prior_que=va_network.get_di_entry_internal_links()[\
		va_id_input_lk_prior_phase].get_set_veh_queue().get_di_obj_veh_queue_at_link()[va_id_input_lk_prior_phase,\
		va_id_output_lk_prior_phase])
		
		#if vehicles can leave the permissive que 
		if veh_can_leave_permis_mv== List_Explicit_Values.initialisation_value_to_one:
			
			#if no prior movement veh is crossing the intersection (!!!! on a deja examine cela,  A VOIR )
			if va_network.get_di_intersections()[va_network.get_di_entry_internal_links()[va_id_input_lk_minor_phase].\
			get_id_head_intersection_node()].get_indicator_prior_mv_cross_intersection()==\
			List_Explicit_Values.initialisation_value_to_zero:
			
				#rep=[nb_veh_leave,t_ev_end_veh_dep,total_potential_nb_dep_veh,state_destin_link]
				rep=self.fct_calcul_nb_veh_they_can_leave_at_t_current_and_assoc_end_dep_time_mi_without_permis_mv(\
				v_t_cur=va_t_current,\
				v_t_unit=va_t_unit,\
				v_queue=va_network.get_di_entry_internal_links()[\
				va_id_input_lk_minor_phase].get_set_veh_queue().get_di_obj_veh_queue_at_link()[\
				va_id_input_lk_minor_phase,va_id_output_lk_minor_phase],\
				v_min_veh_hold_time_in_que=va_min_veh_hold_time_in_que,v_t_end_current_control=va_t_end_current_control,\
				v_netw=va_network,val_t_round_prec=va_t_round_prec)	
			
			#if a prior movement veh is crossing the intersection
			else:	
				nb_veh_leave=List_Explicit_Values.initialisation_value_to_zero
			
				t_ev_end_veh_dep=None
			
				total_potential_nb_dep_veh=va_network.get_di_entry_internal_links()[\
				va_id_input_lk_minor_phase].get_set_veh_queue().get_di_obj_veh_queue_at_link()[\
				va_id_input_lk_minor_phase,va_id_output_lk_minor_phase].get_current_queue_service_rate() - \
				va_network.get_di_entry_internal_links()[\
				va_id_input_lk_minor_phase].get_set_veh_queue().get_di_obj_veh_queue_at_link()[\
				va_id_input_lk_minor_phase,va_id_output_lk_minor_phase].get_current_reached_service_rate()
			
				#we indicate that the dest lk is not saturated, although we havenot examined !
				state_destin_link=List_Explicit_Values.initialisation_value_to_zero
		
				rep=[nb_veh_leave,t_ev_end_veh_dep,total_potential_nb_dep_veh,state_destin_link]
		#if vehicles cannot leave the permissive mov
		else:
			nb_veh_leave=List_Explicit_Values.initialisation_value_to_zero
...
#*****************************************************************************************************************************************************************************************
	...
	#method returning [the number of vehicles that can depart from a  prior que, the associated end dep time, \
	#the total potential number of vehicles in the que, the state of the dest link, indicator of whether   the dest lk is/not saturated], 
	#according to a micro management (veh can leave as as et) 
	#v_vect_nb_veh_to_exam=[nb veh permitted to go ny the sat flow, t_req]
	def fct_calcul_nb_veh_they_can_leave_at_t_current_and_assoc_end_dep_time_mi_from_prior_mv(self,va_t_cur,va_t_unit,va_queue,\
	va_min_veh_hold_time_in_que,va_t_end_current_control,va_netw,va_vect_nb_veh_to_exam,va_t_round_prec=1):
		
		#if no vehicle from a minor movement (related to this prior phase)  is crossing the intersection
		if va_netw.get_di_intersections()[va_netw.get_di_entry_internal_links()[va_queue.get_id_associated_link()].\
		get_id_head_intersection_node()].get_indicator_minor_mv_cross_intersection()==\
		List_Explicit_Values.initialisation_value_to_zero:
		
			#rep=[nb_veh_leave,t_ev_end_veh_dep,total_potential_nb_dep_veh,state_destin_link]
			rep=self.fct_calcul_nb_veh_they_can_leave_at_t_current_and_assoc_end_dep_time_mi_without_permis_mv(\
			v_t_cur=va_t_cur,v_t_unit=va_t_unit,v_queue=va_queue,v_min_veh_hold_time_in_que=va_min_veh_hold_time_in_que,\
			v_t_end_current_control=va_t_end_current_control,v_netw=va_netw,\
			val_t_round_prec=va_t_round_prec)
		
		#if a vehicle of a minor queue is crossing the network then  no vehicle from the prior queue phase can leave
		else:
			total_potential_nb_dep_veh= va_queue.get_current_queue_service_rate() - va_queue.get_current_reached_service_rate()
			
			#if the destination link is not an exit link
			if va_netw.get_di_all_links()[va_queue.get_id_associated_output_link()].get_type_network_link() !=\
			...
			rep=[0,0,total_potential_nb_dep_veh,state_destin_link]
		return rep
		
#*****************************************************************************************************************************************************************************************
	#fct examining whether a vehicle from minor. movement can go, independently of the current ctrl
	#val_prior_que=the prior queues to the current queue
	#it returns 1 if the vehicle can leave zeor otherwise
	def fct_exam_whether_vehs_minor_mov_can_leave_1(self,t_current,val_inters,val_prior_que,val_t_required_cross_inters_minor_phase):
	
		#if a prior veh is crossing the intersection, no vehicle from minor phase can go
		if val_inters.get_indicator_prior_mv_cross_intersection()==List_Explicit_Values.initialisation_value_to_one:
			veh_permitted_to_go_from_prior_mv= List_Explicit_Values.initialisation_value_to_zero
		
		#if no prior veh is crossing the intersection
		else:
	
			#if there are no vehicles in the prior phase
			if len(val_prior_que.get_queue_veh())==List_Explicit_Values.initialisation_value_to_zero:
		
				veh_permitted_to_go_from_prior_mv= List_Explicit_Values.initialisation_value_to_one
			#if there are vehicles in the prior movement, if the required time for the vehicle of minor phase to cross the intersection
			#is at least equalt to the end hold time of the prior phase vehicle 
			elif t_current+val_t_required_cross_inters_minor_phase<\
			val_prior_que.get_queue_veh()[List_Explicit_Values.val_first_element_of_list].get_t_end_veh_hold_time_que():
		...
		return veh_permitted_to_go_from_prior_mv
	
#*****************************************************************************************************************************************************************************************
	#fct examining whether a vehicle from minor. movement can go, independently of the current ctrl
	#val_prior_que=the prior queues to the current queue
	#it returns 1 if the vehicle can leave zeor otherwise
	def fct_exam_whether_vehs_minor_mov_can_leave(self,t_current,val_inters,val_minor_que,val_prior_que):
	
		#if there are vehicles in the minor phase
		if val_minor_que.get_queue_veh()!=[]:
			...
			if val_inters.get_indicator_prior_mv_cross_intersection()==List_Explicit_Values.initialisation_value_to_one:
				...
				#if there are no vehicles in the prior phase
				if val_prior_que.get_queue_veh()==[]:
		
					veh_permitted_to_go_from_prior_mv= List_Explicit_Values.initialisation_value_to_one
				#if there are vehicles in the prior movement, if the prior movement cannot leave the queue because of the hold constraint
				#and the veh from the minor movem can leave now because of the t_end_hold constraint
				elif val_minor_que.get_queue_veh()[List_Explicit_Values.val_first_element_of_list].get_t_end_veh_hold_time_que()<=\
				t_current and \
				val_prior_que.get_queue_veh()[List_Explicit_Values.val_first_element_of_list].get_t_end_veh_hold_time_que()>t_current:
		...
		#if there are no vehicles in the minor phase
		else:
			veh_permitted_to_go_from_prior_mv= List_Explicit_Values.initialisation_value_to_zero
...
#*****************************************************************************************************************************************************************************************
	...
	t_current,t_unit,fct_calcul_nb_and_t_dep_veh,li_param_fct_calcul_nb_and_t_dep_veh,id_que,val_netw,val_t_round_prec,ev_list):
		...
		if nb_and_t_dep_veh[0][List_Explicit_Values.val_first_element_of_list] >List_Explicit_Values.initialisation_value_to_zero:
			...
			message="IN CL_GLOB FUNCT IN FUNCTFct_treat_case_veh_can_go,\
...
#*****************************************************************************************************************************************************************************************
	...
	#method treating the case when no vehicle can leave the queue but there still vehicles in the que
	#we examine in which case an end hold event will be created
	#val_nb_veh_possible_to_leave_from_serv_rate=que service rate-currently reached que service  rate
	
	def fct_treat_case_veh_in_que_none_can_dep_1(self,v_t_current, v_t_end_hold_first_veh_in_que,v_t_unit,v_t_start_new_contr,\
	val_dest_lk_saturated,v_nb_veh_possible_to_leave_from_serv_rate,v_state_first_veh_in_que,\
	v_id_que,v_t_round_prec, v_ev_list):
		...
		if val_dest_lk_saturated==List_Explicit_Values.initialisation_value_to_zero:
			...
			if v_nb_veh_possible_to_leave_from_serv_rate>0:
				...
				if v_state_first_veh_in_que==Cl_Vehicle.TYPE_STATE_VEH["other"]:
					#we calculate of time t_end_hold
