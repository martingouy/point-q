...
(function () {
	...
	function exportCanvas(canvas, format, data) {
		if (!canvas || !format)
			return;
		...
		if (format == "jpg" || format == "png") {
			...
			var mimeType = 'data:application/download,';
			// we convert the JSON to CSV
			var data_csv = '';
			var list_graphs = data.data;
			
			for (var i = 0; i < list_graphs.length; i ++) {

				var list_points = data.data[i].dataPoints;
				data_csv = data_csv + '\n\n' + data.data[i].legendText + '\n';

				for (var j =0; j < list_points.length; j++) {
					if (list_points[j].label) {
						data_csv = data_csv + list_points[j].label + ',' + list_points[j].y + '\n'; 
					}
					else {
						data_csv = data_csv + list_points[j].x + ',' + list_points[j].y + '\n'; 
					}
				}
			}
			var json_to_export = mimeType + encodeURI(data_csv);
			var saved = false;

			var downloadLink = document.createElement("a");
			downloadLink.download = fileName;
			downloadLink.href = json_to_export;
			downloadLink.target = "_blank";
			var e;
		}


		if (!!new Blob() && format != 'json') {
	...
	Chart.prototype._updateOptions = function () {
		...
		if (!this._exportMenu) {
			...
			var exportOption = document.createElement("div");
			exportOption.style.cssText = "padding: 2px 15px 2px 10px"
			exportOption.innerHTML = 'Save as CSV';
			this._dropdownMenu.appendChild(exportOption);
			...
			addEvent(exportOption, "click", function () {
				exportCanvas(_this.canvas, "json", _this._options);
				hide(_this._dropdownMenu);
			}, true);
		}
